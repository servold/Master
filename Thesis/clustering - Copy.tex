...
\begin{equation}
	\min_{x^1, x^2, \ldots, x^k \in \mathbb{R}^n} \left\lbrace \sum\limits_{i=1}^{m} \min\limits_{1 \leq l \leq k} d(x^l,a^i) \right\rbrace , \label{StateEq30}
\end{equation}
where $d(u,v) = \|u - v\|$. \\
...
Set $\rho_1 = \frac{1}{2} \min \left\lbrace \alpha(t), L(t) \right\rbrace$, and again \Cref{StateEq17} assures that $\rho_1 > 0$. Combined with (\ref{StateEq37}) and (\ref{StateEq39}) we have that
\begin{equation*}
\begin{aligned}
	\rho_1 \|z(t+1) &- z(t)\|^2 
	 = \rho_1 \left( \|w(t+1) - w(t)\|^2 + \|x(t+1) - x(t)\|^2  \right) \leq \\
...
Next we prove the subgradient lower bounds for iterates gap property of the sequence generated by (Alg-Name), we start with a useful lemma.
...
\begin{aligned}
	\|d^i(x(t+1)) - d^i(x(t))\| 
	&\leq \left[ \sum_{l=1}^{k} \abs{ \| x^l(t+1) - a^i\| - \|x^l(t) - a^i\| }^2 \right]^\frac{1}{2} \\
	&\leq \left[ \sum_{l=1}^{k}  \| \left( x^l(t+1) - a^i \right) - \left( x^l(t) - a^i\right)\|^2 \right]^\frac{1}{2} \\
...
\begin{proof}
Repeating the steps of the proof in case of KPALM yields that 
\begin{equation}
	\gamma(t+1) := \left( \left( d^i(x(t+1)) + u^i(t+1) \right)_{i=1, \ldots ,m}, \nabla_x H(w(t+1),x(t+1)) \right) \in \partial \Psi(z(t+1)) , \label{StateEq41}
\end{equation}
where for all $1 \leq i \leq m, \: u^i(t+1) \in \partial \delta_{\Delta}(w^i(t+1))$ such that
\begin{equation}
	d^i(x(t)) + \alpha_i(t) \left( w^i(t+1) - w^i(t) \right) + u^i(t+1) = \bf{0} . \label{StateEq42}
\end{equation}
Plugging (\ref{StateEq42}) into (\ref{StateEq41}), and taking norm yields
\begin{equation*}
\begin{aligned}
	\| \gamma(t+1) \|
...
\end{aligned}
\end{equation*}
where the last inequality follows from \Cref{StateEq40}, and $\overline{\alpha}(t) = \max\limits_{1 \leq i \leq m} \alpha_i(t)$. \\ %Hence it is sufficient to find $c>0$ and bound $ \| \nabla_x H(w(t+1),x(t+1)) \| \leq c\|x(t+1) - x(t)\|$.
Applying lemma 4.1 [reference to Weiszfeld paper] with respect to $H_l(w,\cdot)$ yields
\begin{equation*}
\begin{split}
	H_l(w,T_l(w,x)) \leq H_l(w,x) + \left\langle \nabla_{x^l} H_l(w,x)), H_l(w,T_l(w,x)) - H_l(w,x) \right\rangle \\ + \frac{L_l(w,x)}{2} \|T_l(w,x) - x^l\|^2, \quad \forall x \in (\mathbb{R} \setminus \mathcal{A})^k.
\end{split}
\end{equation*}
Close review of the proof of lemma 4.1 reveals that we can switch $x^l$ with $T_l(w,x)$ and get
\begin{equation*}
\begin{split}
	H_l(w,x) \leq H_l(w,T_l(w,x)) + \left\langle \nabla_{x^l} H_l(w,T_l(w,x)), H_l(w,x) - H_l(w,T_l(w,x)) \right\rangle \\ + \frac{L_l(w,T_l(w,x))}{2} \|x^l - T_l(w,x)\|^2, \quad \forall x \in (\mathbb{R} \setminus \mathcal{A})^k.
\end{split}
\end{equation*}
Summing the last two inequalities and rearranging lead to
\begin{equation*}
\begin{split}
	\left\langle \nabla_{x^l} H_l(w,T_l(w,x)) \right. &- \left. \nabla_{x^l} H_l(w,T_l(w,x)), H_l(w,T_l(w,x)) - H_l(w,x) \right\rangle \leq \\
	&\leq \frac{L_l(w,x) + L_l(w,T_l(w,x))}{2} \|T_l(w,x) - x^l\|^2, \quad \forall x \in (\mathbb{R} \setminus \mathcal{A})^k.
\end{split}
\end{equation*}
Thus, it follows that
\begin{equation}
\begin{split}
	\| \nabla_{x^l} H_l(w,T_l(w,x)) &- \nabla_{x^l} H_l(w,T_l(w,x)) \| \leq \\ 
	&\leq \frac{L_l(w,x) + L_l(w,T_l(w,x))}{2} \|T_l(w,x) - x^l\|, \quad \forall x \in (\mathbb{R} \setminus \mathcal{A})^k.  \label{StateEq43}
\end{split}
\end{equation}
With the last result we shall bound the $\| \nabla_x H(w(t+1),x(t+1)) \| \leq c\|x(t+1_ - x(t)\|$, for some constant $c>0$, as follows
\begin{equation*}
\begin{aligned}
	&\| \nabla_x H(w(t+1),x(t+1)) \| \leq \sum\limits_{l=1}^{k} \| \nabla_{x^l} H_l(w(t+1),x(t+1)) \| \\
	&\leq \sum\limits_{l=1}^{k} \| \nabla_{x^l} H_l(w(t+1),x(t)) \| + \sum\limits_{l=1}^{k} \| \nabla_{x^l} H_l(w(t+1),x(t+1)) - \nabla_{x^l} H_l(w(t+1),x(t))\| \\
	&\leq \sum\limits_{l=1}^{k} L_l(w(t+1),x(t)) \|x^l(t+1) - x^l(t)\| + \sum\limits_{l=1}^{k} \frac{L_l(w(t+1),x(t)) + L_l(w(t+1),x(t+1))}{2} \|x^l(t+1) - x^l(t)\| ,
\end{aligned}
\end{equation*}
where the last inequality follows from (\ref{StateEq36}) and (\ref{StateEq43}).
Therefore, denote $\overline{L}(t) = \\ \frac{1}{2} \max\limits_{1 \leq l \leq k} \left\lbrace 3L_l(w(t+1),x(t)) + L_l(w(t+1),x(t+1)) \right\rbrace$, and set $\rho_2 = m + m\overline{\alpha}(t) + k\overline{L}(t)$, and the result follows.
\end{proof}
