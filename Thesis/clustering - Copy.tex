...
\begin{proof}
We introduce the following function $H^l_{\varepsilon}: \mathtt{R}^{km} \times \mathbb{R}^{nk} \rightarrow \mathbb{R}$ defined by
\begin{equation*}
...
\end{equation*}
Summing the last inequality over $l=1,2, \ldots, k$, and using the fact that $H_{\varepsilon}(w,x) = \sum_{l=1}^{k} H^l_{\varepsilon}(w,x)$ for all $w \in {\Delta}^m$ and $x \in \mathbb{R}^{nk}$, yields
\begin{equation*}
\begin{aligned}
	H_{\varepsilon}(w(t+1),x(t+1)) 
	&\leq H_{\varepsilon}(x(t)) + \sum\limits_{l=1}^{k} \frac{L^l_{\varepsilon}(x(t))}{2} \|x^l(t+1)-x^l(t)\|^2 \\
	&+ \sum\limits_{l=1}^{k} \left\langle \nabla_{x^l} H^l_{\varepsilon}(x(t)), x^l(t+1)-x^l(t) \right\rangle  .
\end{aligned}
\end{equation*}
Replacing the last term with the following compact form
\begin{equation*}
	\sum\limits_{l=1}^{k} \left\langle \nabla_{x^l} H^l_{\varepsilon}(x(t)), x^l(t+1)-x^l(t) \right\rangle = \left\langle \nabla_x H_{\varepsilon}(w(t+1),x(t)), x(t+1)-x(t) \right\rangle ,
\end{equation*}
