...
Before proceeding towards the algorithm that is based on PALM theory, we need to develop some useful results.
...
\begin{enumerate}[(i)]
	...
	\end{equation*}
	Multiplying the last inequality by $w_i$ and summing over $i=1,2, \ldots, m$, and the results follows. 
	\item The function $x \mapsto h(x,y)$ is strongly convex and its unique minimizer is determined by the optimality equation
	\begin{equation*}
...
The following lemma proofs are based on the properties of the auxiliary function $H_{\epsilon}$, and they are similar to the proofs in [BS2015], hence we will just state them here.
...
\end{equation*}
It is clear that $x \mapsto \widetilde{h_{\epsilon}}(x,y)$ is still quadratic function with associated matrix $L_{\epsilon}(y)\mathbf{I}$. Therefore, we can write
\begin{equation}
\begin{aligned}
...
\end{aligned}
\end{equation*}
Thus, using the definition of $\widetilde{f_{\epsilon}}$ and the face that $\nabla \widetilde{f_{\epsilon}}(y) = \nabla f_{\epsilon}(y) - \nabla f_{\epsilon}(y^0)$, yields that
\begin{equation*}
...
and
\begin{equation*}
 T_{\epsilon}^{w_l}(x^l) = \frac{ \sum\limits_{i=1}^m \frac{w^i_l a^i}{\left( \|x^l-a^i\|^2 + {\epsilon}^2 \right)^{1/2}} }{L_{\epsilon}^{w_l}(x^l)}.
\end{equation*}
...
\begin{equation}
	\frac{\alpha(t)}{2} \|w(t+1) - w(t)\|^2 \leq H_{\epsilon}(w(t),x(t)) - H_{\epsilon}(w(t+1),x(t)) , \\ \label{StateEq37}
\end{equation}
where $\alpha(t) = \min\limits_{1 \leq i \leq m} \left\lbrace \alpha_i(t) \right\rbrace$.

Applying \Cref{StateEq64} with respect to $H_{\epsilon}^{w_l(t+1)}(\cdot)$ yields
\begin{equation*}
...
Denote ${L(t)}= \min\limits_{1 \leq l \leq k} \left\lbrace L_{\epsilon}^{w_l(t+1)}(x^l(t)) \right\rbrace$. Summing (\ref{StateEq38}) over $l = 1,2, \ldots ,k$ leads to
\begin{equation}
	\begin{split}
	...
	& \leq \sum\limits_{l=1}^{k} \left( H_{\epsilon}^{w_l(t+1)}(x^l(t)) - H_{\epsilon}^{w_l(t+1)}(x^l(t)) \right) \\
	& = H_{\epsilon}(w(t+1),x(t)) - H_{\epsilon}(w(t+1),x(t+1)) .
	\end{split} \label{StateEq39}
\end{equation}

Set $\rho_1 = \frac{1}{2} \min \left\lbrace \alpha(t), L(t) \right\rbrace$, and note that \Cref{StateEq17} assures that $\rho_1 > 0$. Combining (\ref{StateEq37}) and (\ref{StateEq39}) yields
\begin{equation*}
\begin{aligned}
	\rho_1 \|z(t+1) &- z(t)\|^2 
	 = \rho_1 \left( \|w(t+1) - w(t)\|^2 + \|x(t+1) - x(t)\|^2  \right) \leq \\
...
\begin{proof}
Define $\psi(t)=\sqrt{t + {\epsilon}^2}$, for $t \geq 0$. Using Lagrange mean value theorem over $a > b \geq 0$ yields
\begin{equation*}
...
\begin{aligned}
	...
	&\leq (1/2\epsilon) \abs{ \|x^l-a^i\|^2 + {\epsilon}^2 - \left( \|y^l-a^i\|^2 + {\epsilon}^2 \right) } \\
	&= (1/2\epsilon) \abs{\|x^l-a^i\|^2 - \|y^l-a^i\|^2} \\
	&= (1/2\epsilon) \abs{\|x^l-a^i\| + \|y^l-a^i\|} \cdot \abs{\|x^l-a^i\| - \|y^l-a^i\|} \\
	&\leq \frac{d_{\mathcal{A}}\|x^l-y^l\|}{\epsilon} .
\end{aligned}
\end{equation*}
Hence,
\begin{equation*}
\begin{aligned}
	...
	&\leq \left[ \sum_{l=1}^{k} \left( \frac{d_{\mathcal{A}}\|x^l-y^l\|}{\epsilon} \right)^2 \right]^\frac{1}{2} \\
...
\end{equation*}
applying \Cref{StateEq65} with respect to $H_{\epsilon}^{w_l(t+1)}(\cdot)$ and plugging into (\ref{StateEq47}) yields
\begin{equation*}
\begin{aligned}
...
\end{aligned}
\end{equation*}
Therefore, denote $\overline{L}(t) = \max\limits_{1 \leq l \leq k} \left\lbrace L_{\epsilon}^{w_l(t+1)}(x^l(t)) + \frac{2 L_{\epsilon}^{w_l(t+1)}(x^l(t)) L_{\epsilon}^{w_l(t+1)}(x^l(t+1))}{L_{\epsilon}^{w_l(t+1)}(x^l(t)) + L_{\epsilon}^{w_l(t+1)}(x^l(t+1))} \right\rbrace$, and set \\ $\rho_2 = m + m\overline{\alpha}(t) + k\overline{L}(t)$, and the result follows.
\end{proof}
