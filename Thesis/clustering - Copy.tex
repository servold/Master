...
\end{equation}
with $v^* = 1/\sqrt{\norm{u}^2 + {\varepsilon}^2}$. Note that $v^* \in I := \left[ 1/\kappa , 1/\varepsilon \right]$, where $\kappa = \sqrt{2} \max\{d_{\mathcal{A}},\varepsilon\}$.
...
\end{equation}
where $v = \left(v^1, v^2, \ldots v^m \right) \in I^{km}$. For all $i=1,2,\ldots,m$ we define $h^i_{\varepsilon}: \rr^{mk} \times \rr^{nk} \rightarrow \rr^{k}$ by
\begin{equation*}
...
\begin{equation}
	v(t+1) = \arg\!\min \{ B_{\varepsilon}(v,w(t),x(t)) \mid v \in \rr^{mk} \} . \label{v_step}
\end{equation}
