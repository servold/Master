...
I believe that with \Cref{StateLemma_x_bounds_w} we can show the usual sufficient decrease property of PALM in the following manner. As we discussed the following inequality still holds in the case of $\alpha = 0$
\begin{equation}
	\frac{\underline{\beta}}{2} \|x(t+1)-x(t)\|^2 \leq H(w(t),x(t)) - H(w(t+1),x(t+1)) . \label{StateEq70}
\end{equation}
Setting $\rho_1 = \underline{\beta} /2(1 + mc^2)$, where $c$ is as in \Cref{StateLemma_x_bounds_w}, and we can write
\begin{equation*}
\begin{aligned}
...
\end{aligned}
\end{equation*}
where the first inequality follows from \Cref{StateLemma_x_bounds_w}, the second follows from (\ref{StateEq70}), and the last equality follows from the fact that $G(w(t))=0$, for all $t \in \mathbb{N}$. I think this proves the sufficient decrease part.
...
\begin{figure}[h!]
    ...
    \label{fig:dynamic_alpha}
\end{figure} 

\Cref{fig:dynamic_alpha} shows that dynamic $\alpha$ parameter with fast decrease in its value, such as $\alpha_i(t)=\frac{diam(\mathcal{A})}{2^{t-1}}$, may achieve better clustering function values (that is lower), and increase the rate of conversion.

\begin{figure}[h!]
    \centering
    \includegraphics{psi_algs_comparison}
    \caption{Comparison of $\Psi$ values for KMEANS, KMEANS++ and KPALM.}
     \label{fig:algs_comparison}
\end{figure} 

In \Cref{fig:algs_comparison} we made a comparison between KPALM with dynamic $\alpha$ parameter, $\alpha_i(t)=\frac{diam(\mathcal{A})}{2^{t-1}}$, with KMEANS amd KMEANS++. It demonstrates that KPALM can reach lower $\Psi$ values then KMEANS, and these are similar to the values achieved with KMEAN++.

%\begin{figure}[h!]
%    \centering
%    \includegraphics[width=1\textwidth]{relative_delta_algs_comparison}
