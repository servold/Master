...
Let $(C(t), x(t))$ be the clusters and centers KMEANS returns. Denote an open neighborhood of $x(t)$ by $U = B\left( x^1(t),\frac{r(t)}{2}\right) \times  B\left( x^2(t),\frac{r(t)}{2}\right) \times \cdots \times B\left( x^l(t),\frac{r(t)}{2} \right)$, then for any $x \in U$ we have $\underline{C^l}(x)= C^l(t)$ for all $1 \leq l \leq k$.
\end{proposition}
...
Now, since $C(t)$ is a partition of $\mathcal{A}$ then $\underline{C^l}(x)= C^l(t)$ for all $1 \leq l \leq k$.
\end{proof}
