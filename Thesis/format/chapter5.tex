...
We outline the observations that arose from the conducted numerical experiments:
\begin{itemize}
	...
	\item In datasets with outliners in some of the clusters, the clustering obtained with $\varepsilon$-KPALM is more similar to the desired clustering, in terms of clustering metrics, then the clusterings obtained via the squared Euclidean algorithms. This is witnessed from the sparse Gaussians example in \Cref{fig:gaussians}(\ref{fig:sparse_gaussians}), and the comparison made in \Cref{fig:gaussians_metrics}(\ref{fig:sparse_gaussians_metrics}).
\end{itemize}