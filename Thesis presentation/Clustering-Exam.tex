...
\\ 2\sum\limits_{i=1}^{m} w^i_l &\mbox{if } j = l, \quad 1 \leq j,l \leq k, \end{cases}$
	...
	\begin{frame}{$\varepsilon$-KPALM Subgradient Lower Bound The Iterates Gap Proof-Contd.}
		...
		\begin{align*}
			...
			&\leq \frac{m}{\varepsilon} \sum\limits_{l=1}^{k} \|x^l(t+1)-x^l(t)\| + \sum\limits_{l=1}^{k} \gamma^l(t) \|x^l(t+1)-x^l(t)\| ,\quad (*) 
		\end{align*}
		}}\\
		where the last inequality follows from Lemma \ref{State_L_bounds}(ii) and \Cref{f_eps_lipschitz_prop} combined with the (\ref{H_eps_f_eps}) observation using\\
		\scalebox{.8}{\parbox{\linewidth}{%
		\begin{equation*}
